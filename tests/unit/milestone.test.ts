...
describe('milestone complete command', () => {
  ...
  test('handles missing ROADMAP.md gracefully', () => {
    // Only STATE.md â€” no ROADMAP.md, no REQUIREMENTS.md
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      `# State\n\n**Status:** In progress\n**Last Activity:** 2025-01-01\n**Last Activity Description:** Working\n`
    );
  ...
  test('non-numeric directory is excluded when milestone scoping is active', () => {
    ...
    // Non-phase directory â€” should be excluded
  ...
  test('large phase numbers (456, 457) scope correctly', () => {
    ...
    // Phase 45 from prior milestone â€” should not match
...
`;
  ...
  test('idempotent â€” re-marking already-complete requirement does not corrupt', () => {
    writeRequirements(tmpDir, STANDARD_REQUIREMENTS);
    ...
    const content = readRequirements(tmpDir);
    // File should not be corrupted â€” no [xx] or doubled markers
  ...
  test('missing REQUIREMENTS.md returns expected error structure', () => {
    // createTempProject does not create REQUIREMENTS.md â€” so it's already missing
